...
function out = forces_moments(x, delta, wind, P)
    ...
    noseb=[1.25;0;0];
    tailb=[2.5;0;0];
    wingtip1v=Rvb*wingtip1b;
    wingtip2v=Rvb*wingtip2b;
    nosev=Rvb*noseb;
    tailv=Rvb*tailb;
    
    if any([wingtip1v(3),wingtip2v(3),nosev(3),tailv(3)]+pd>0)
        error('Crash!!')
    end
%     

    persistent deflprev;
    persistent defldotprev;
    ...
    % the landing gear
    cosang=[0;0;-1]'*Rvb*[0;0;-1]; %angle between inertial down and body down
    velv=Rvb*[u;v;w]; % velocity of plane in inertial
    velv=[velv(1:2);0]; % get rid of z component
    if norm(velv)<1
        atRest=1;
    else
        atRest=0;
    end
    forwardv=Rvb*[1;0;0]; % body x in inertial
    forwardv=[forwardv(1:2);0]; % get rid of z component
    velvfor=velv'*forwardv*forwardv; % velocity in the forward direction on the ground
    velvslide=velv-velvfor; % velocity in the side direction on the ground
    forPerc=norm(velvfor)/norm(velv);
    slidePerc=norm(velvslide)/norm(velv);
...
%     forceFric=[0;0;0];
    ...
    defldotCur=[0;0;0];
    for ii=1:3
        wheelPosB=P.wheelpositions{ii};
        wheelPosV=Rvb*wheelPosB+[pn;pe;pd];
        if (wheelPosV(3)>0), deflt(ii)=wheelPosV(3)/cosang;else deflt(ii)=0; end
        wheelPosB(3)=wheelPosB(3)-deflt(ii);
        defldotCur(ii)=(deflt(ii)-deflprev(ii))/P.Ts;
        defldotprev(ii)=defldotprev(ii)*.9+.1*defldotCur(ii);
        FsuspB=[0;0;-deflt(ii)*P.ksusp-P.Bsusp*defldotprev(ii)];
        
%         FsuspB=[0;0;-deflt(ii)*P.ksusp-P.Bsusp*(deflt(ii)-deflprev(ii))/P.Ts];
        
        %disp([num2str(-P.Bsusp*(deflt(ii)-deflprev(ii))/P.Ts)]);% Force due to suspension in body
        FsuspV=Rvb*FsuspB; %Force due to suspension in inertial
        Fn=abs(FsuspV(3)); %normal force
        
%         FfricV=FfricV-Fn*P.kfric*brake1*forPerc*velvfor/norm(velvfor)-...
%             Fn*P.kfric*slidePerc*velvslide/norm(velvslide);
%         torqueSusp=torqueSusp+cross(wheelPosB,FsuspB);
%         forceSusp=forceSusp+(-wheelPosB')*(FsuspB)/norm(wheelPosB)^2*-wheelPosB;

        FfricV=-Fn*P.kfric*brake1*forPerc*velvfor/norm(velvfor)-...
            Fn*P.kfric*slidePerc*velvslide/norm(velvslide);
        FfricB=Rbv*FfricV;
        torqueSusp=torqueSusp+cross(wheelPosB,FsuspB+FfricB);
        forceSusp=forceSusp+(-wheelPosB')*(FsuspB+FfricB)/norm(wheelPosB)^2*-wheelPosB;
            
    end
%     if(any(abs(deflt-deflprev)))
%         deflt-deflprev
%     end
    defl1=deflt(1);
    defl2=deflt(2);
...
%     if any([defl1,defl2,defl3]>P.maxDefl)
%         error('Crash Defl!!')
%     end
    w_n = w_ns+gustv(1);
    w_e = w_es+gustv(2);
    ...
    Force=Force+forceSusp';
%     if atRest 
%         ForceV=Rbv*Force';
%         if abs(ForceV(1))<abs(FfricV(1)) && sign(ForceV(1))~=sign(FfricV(1))
%             ForceV(1)=0;
%         end
%         if abs(ForceV(2))<abs(FfricV(2)) && sign(ForceV(2))~=sign(FfricV(2))
%             ForceV(2)=0;
%         end
%         Force=(Rvb*ForceV)';
%         
%     else
%         Force=Force+(Rvb*FfricV)';
%     end
%         
    %forceWheels
